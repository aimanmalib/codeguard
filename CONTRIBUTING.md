...
Thanks for your interest in improving CodeGuard. This project is a
provider-agnostic, 7-agent code review pipeline that runs on any
...
```bash
git clone https://github.com/aimanmalib/codeguard.git
cd codeguard
python -m venv .venv && source .venv/bin/activate
pip install -e ".[dev]"
pytest          # 49 tests should pass
```
...
3. Run the full local gate before pushing:
   ...
   pytest --cov=codeguard          # tests + coverage
   ```
...
CodeGuard talks to any OpenAI-compatible `/chat/completions` endpoint, so most
providers need only a preset entry. In `src/codeguard/core/config.py`, add to
`PROVIDER_PRESETS`:
...
- Add a new output format (SARIF, GitHub annotations)
- Improve an agent's review prompt
- Expand language detection in `cli._EXT_TO_LANG`

## Code style

- Python 3.10+ with type hints
- `ruff` for linting and formatting (config in `pyproject.toml`)
- Code review agents use a low temperature (0.3) for deterministic severity
- Public APIs get docstrings

## Reporting bugs / requesting features

Use the issue templates (bug report / feature request). Include repro steps,
your provider/model, and the CodeGuard version for bugs.
